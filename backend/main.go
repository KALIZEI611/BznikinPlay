...
func initDB() error {
    ...
    // Если переменные не найдены, используем значение из интерфейса Railway
    if host == "" {
        ...
        dbname = "railway"
        log.Println("Using hardcoded database settings")
    }
    ...
    log.Printf("Connecting to: host=%s port=%s", host, port)
